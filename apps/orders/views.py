...
def order_import(request):
    ...
    for item in items:
        ...
        mini_qs = MinifigurePart.objects.filter(minifigure__owner=request.user, part_number=item["part_number"], quantity__gt=models.F("owned_quantity"), is_spare=False).filter(models.Q(minifigure__lego_set__isnull=True) | models.Q(minifigure__lego_set__owner=request.user, minifigure__lego_set__deleted_at__isnull=True)).select_related("minifigure", "minifigure__lego_set").order_by("minifigure__lego_set_id", "minifigure_id", "pk")
        candidates = []
        ...
        for candidate in mini_qs:
            ...
            if item["color"] and not color_match:
                continue
            candidates.append({"kind": "minifigure_part", "id": candidate.pk, "set_name": (candidate.minifigure.lego_set.set_number if candidate.minifigure.lego_set_id else "Einzeln hinzugef\u00fcgt"), "open": candidate.missing_quantity, "quality": "EXACT" if color_match else "POSSIBLE"})
