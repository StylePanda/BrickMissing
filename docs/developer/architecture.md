...
`apps.documentation` stellt die freigegebenen Markdown-Dateien aus `docs/` unter `/docs/` bereit. Eine feste Zuordnung von URL-Slug zu Datei verhindert freie Dateipfade. Die Benutzerdokumentation ist öffentlich; alle `developer`-Routen einschließlich der Versionierungsseite erfordern serverseitig `request.user.is_staff` und antworten sonst mit 404. Die Navigation zeigt den Entwicklerabschnitt nur Administratoren. Dokumentationsantworten tragen `Cache-Control: private, no-store` und `Vary: Cookie`, damit keine Administratornavigation aus einem gemeinsamen Cache ausgeliefert wird. `markdown-it-py` rendert die Dateien bei der Anfrage mit deaktiviertem Raw HTML; der Renderer prüft Links und leitet interne Markdown-Links auf Dokumentationsrouten um. Die Ansicht verwendet das gemeinsame Base-Template mit eigener responsiver Seitennavigation; es gibt keine zweite HTML-Kopie der Inhalte.
